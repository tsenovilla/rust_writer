...
#[test]
fn item_without_docs_item_struct() {
	...
	let mut output = item_without_docs(&item_struct);

	let expected_attrs: Vec<Attribute> = vec![parse_quote!(#[some_attr])];

	assert_eq!(*output.attrs_mut().unwrap(), expected_attrs);
}
