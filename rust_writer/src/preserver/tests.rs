...
fn apply_preservers_works() {
	TestBuilder::default().with_complete_file().with_preserved_file().execute(|builder| {
		...
		assert_eq!(preserved_code, apply_preservers(code, vec![preserver1, preserver2, preserver3]));
	});
}
